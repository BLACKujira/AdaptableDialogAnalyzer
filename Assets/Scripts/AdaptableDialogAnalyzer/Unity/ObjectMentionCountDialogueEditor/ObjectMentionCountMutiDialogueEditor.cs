...
namespace AdaptableDialogAnalyzer.Unity
{
    public abstract class ObjectMentionCountMutiDialogueEditor : CountDialogueEditor
    {
        ...
        ObjectMentionedCountMutiMatrix mentionedCountMutiMatrix;
        protected ObjectMentionedCountMutiMatrix MentionedCountMutiMatrix => mentionedCountMutiMatrix;

        public virtual void Initialize(ObjectMentionedCountMutiMatrix mentionedCountMutiMatrix)
        {
            this.mentionedCountMutiMatrix = mentionedCountMutiMatrix;
        ...
        protected override void InitializeSpeechBubble(BasicTalkSnippet basicTalkSnippet, SpeechBubbleButton speechBubbleButton)
        {
            ...
            speechBubbleButton.button.onClick.AddListener(() =>
            {
                ...
                oMCMInputInt.Initialize(basicTalkSnippet.content, countMatrix.mentionedCountRows[basicTalkSnippet.speakerId][basicTalkSnippet.refIdx], countMatrix.unidentifiedMentionsRow[basicTalkSnippet.refIdx],
                    (i) =>
                    {
                        countMatrix.unidentifiedMentionsRow.RemoveGrid(basicTalkSnippet.refIdx); // 移除未识别的匹配
                        countMatrix.mentionedCountRows[basicTalkSnippet.speakerId][basicTalkSnippet.refIdx].overrideCount = i; //不使用匹配计数，覆盖计数
            ...
            {
                speechBubbleWithIntAndButton.txtInt.text = countMatrix.mentionedCountRows[basicTalkSnippet.speakerId][basicTalkSnippet.refIdx].Count.ToString();
            }
        }
    }
}