...
namespace AdaptableDialogAnalyzer.Unity
{
    ...
    /// 单角色模式选择器
    /// </summary>
    public class ChapterSelectorOMCMChara : ChapterSelector
    {
        ...
        public void Initialize(ObjectMentionedCountMutiManager mentionedCountManager, int speakerId)
        {
        ...
        {
            countMatrices = countMatrices.Where(cm => cm is ObjectMentionedCountMutiMatrix omcm && omcm[speakerId] != null && omcm[speakerId].serifCount > 0).ToList();

            if (togHideUnmatched.isOn)
            {
                countMatrices = countMatrices.Where(cm => cm is ObjectMentionedCountMutiMatrix omcm && omcm[speakerId] != null && omcm[speakerId].Count > 0).ToList();
            }
        ...
        {
            ObjectMentionedCountMutiMatrix mentionedCountMatrix = countMatrix as ObjectMentionedCountMutiMatrix;
            ...
            else chapterItem.SetData(countMatrix.chapterInfo, mentionedCountMatrix[speakerId].serifCount);

            chapterItem.button.onClick.AddListener(() =>
            {
                ObjectMentionCountMutiDialogueEditorChara dialogueEditor = window.OpenWindow<ObjectMentionCountMutiDialogueEditorChara>(dialogueEditorPrefab);
