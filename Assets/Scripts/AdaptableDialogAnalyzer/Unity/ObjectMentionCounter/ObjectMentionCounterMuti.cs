...
{
    public class ObjectMentionCounterMuti : MonoBehaviour
    {
        [Header("Components")]
        public ObjectMentionCounterMuti_DisplayArea displayArea;
        ...
        /// 已经统计过的剧情
        /// </summary>
        ObjectMentionedCountMutiManager mentionedCountMutiManager = new ObjectMentionedCountMutiManager();
        public ObjectMentionedCountMutiManager MentionedCountMutiManager => mentionedCountMutiManager;
        ...
        private void Update()
        {
            int counted = mentionedCountMutiManager.mentionedCountMatrices.Count;
        ...
        void Count(Chapter[] chapters)
        {
            ...
            {
                ObjectMentionedCountMutiMatrix mentionedCountMutiMatrix = CountChapter(chapter);
                mentionedCountMutiManager.mentionedCountMatrices.Add(mentionedCountMutiMatrix);

                string savePath = Path.Combine(saveFolder, chapter.ChapterID + ".omcmm");
                mentionedCountMutiMatrix.SerializeAndSave(savePath);
        ...
        /// </summary>
        ObjectMentionedCountMutiMatrix CountChapter(Chapter chapter)
        {
            ObjectMentionedCountMutiMatrix mentionedCountMutiMatrix = new ObjectMentionedCountMutiMatrix(chapter);
            mentionedCountMutiMatrix.Chapter = chapter;
            ...
            foreach (var talkSnippet in talkSnippets)
            {
                ...
                CountSnippet(matchSnippet, mentionedCountMutiMatrix);
            }

            return mentionedCountMutiMatrix;
        }

        /// <summary>
        /// 统计片段，统计后直接添加到MentionedCountMatrix中
        /// </summary>
        void CountSnippet(BasicTalkSnippet talkSnippet, ObjectMentionedCountMutiMatrix mentionedCountMutiMatrix)
        {
            ...
            //记录台词
            mentionedCountMutiMatrix.AddSerifCount(talkSnippet.SpeakerId, 1);

            //首先判断通用昵称
            foreach (var regex in objectNameDefinition.CommonNameList)
            {
                Regex regexObject = regexDictionary[regex]; //获取正则表达式对象
                MatchCollection matchCollection = regexObject.Matches(talkSnippet.Content);
                foreach (var match in matchCollection)
                {
                    mentionedCountMutiMatrix.AddRegexCapture(talkSnippet.SpeakerId, talkSnippet.RefIdx, (Match)match);
            ...
            foreach (var regex in objectNameDefinition.UnidentifiedNameList)
            {
                ...
                foreach (var match in matchCollection)
                {
                    mentionedCountMutiMatrix.unidentifiedMentionsRow.AddRegexCapture(talkSnippet.RefIdx, (Match)match);
